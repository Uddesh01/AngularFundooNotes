...
export class ArchiveContainerComponent implements OnInit {
  ...
  updateNotesList($event: { action: string, data: { title: string, description: string, noteID: number } }): void {
    ...
    //work is pending 
      ...
      this.noteService.trash($event.data.noteID).subscribe(res => {
        console.log($event.data.noteID)
        this.updateList.emit({ action: "trash", data: { title:$event.data.title, description: $event.data.description, noteID: $event.data.noteID } })
      }, err => console.log(err))
      this.notesList = this.notesList.filter(note=>note.noteID!==$event.data.noteID);
    }
  }
